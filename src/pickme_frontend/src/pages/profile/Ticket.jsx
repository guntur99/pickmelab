    ...
    const handleTransferTicket = (e) => {
        ...
        pickme_backend.checkUsername(receiverUsername).then((res) => {
            ...
            if (available.length > 0) {
                ...
                setIsLoading(true);
                pickme_backend.transferTicket(available[0].internet_identity, receiverUsername, ticket.event_id, ticket.event_title, ticket.category, 1, parseInt(ticket.price), parseInt(ticket.icp_price), ticket.discount, ticket.uuid).then((res) => {
                    if (res) {
                        setIsLoading(false);
